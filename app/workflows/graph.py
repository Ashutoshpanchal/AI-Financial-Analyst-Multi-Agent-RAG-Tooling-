...
Full flow:
    START
      └── router_agent  (classify query)
            ...
            ├── rag         → rag_agent → planner → aggregator → critic → END
            ├── computation → computation_agent → planner → aggregator → critic → END
            └── hybrid      → parallel(rag + computation) → planner → aggregator → critic → END
"""
...
def build_graph() -> StateGraph:
    ...
    # All paths converge at planner
    builder.add_edge("rag_agent",           "planner_agent")
    builder.add_edge("computation_agent",   "planner_agent")
    builder.add_edge("parallel_agent",      "planner_agent")
